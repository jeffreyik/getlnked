...
const BlockCategory = ({ category }: any) => {
  ...
  const addComponent = (objects: any, parent: any, object: any) => {
    return objects?.map((item: any) => {
      if (item?.id === parent.id) {
        ...
        return {
          ...item,
          children: addComponent(item.children, parent, object),
        };
      }
      return item;
    });
  ...
  return (
    <div className="w-full">
      ...
      <div
        className={`flex justify-between transition-all flex-wrap gap-2 ${
          toggleCategory ? "h-fit" : "h-0"
        } overflow-hidden`}
      >
        {components.map(
          (component: any) =>
            component.category === category && (
              <Block
                handleClick={() => addComponentToState(component?.config)}
                key={component.name}
                name={component.name}
